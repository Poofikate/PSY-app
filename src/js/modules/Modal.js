...
export default class Modal extends ClassToggler {
  constructor(options) {
    ...
    this.dontCloseOnClick =
      this.$el.hasAttribute('data-dont-close-on-click') || false;
    this.$el = options.$el;
    this._zIndex = 10;
    this.init();
  }
  ...
  init() {
    ...
    this.$el.addEventListener('click', (e) => {
      if (this.dontCloseOnClick) return false;
  ...
  open(e) {
    const isTargetOfChild = e && Helper.isParentHasClass(e.target, 'tooltip');
    if (isTargetOfChild) return false;
